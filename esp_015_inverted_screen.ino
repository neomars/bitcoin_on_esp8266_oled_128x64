...
// --- Les bitmaps WiFi originaux ont été supprimés car non compatibles / non utilisés ---
// Si vous souhaitez une animation, vous devrez créer des bitmaps spécifiques Adafruit GFX
// ou des séquences de dessin avec les primitives graphiques.
...
const long intervalPage = 5000; // Changer de page toutes les 5 secondes
...
// --- Ancien paramètres du bouton pour forcer le portail de configuration (supprimés pour ESP-01) ---
...
// --- Fonctions de dessin de symboles WiFi ---
...
void updateCryptoData() { // Renommé pour être plus générique
  ...
  if (WiFi.status() != WL_CONNECTED) { // Vérifie la connexion WiFi avant de tenter une requête
    Serial.println("WiFi non connecté, impossible de récupérer les données crypto.");
...
void setup() {
  ...
  // --- Initialisation de l'écran OLED pour ESP-01 (SDA=GPIO0, SCL=GPIO2) ---
  // L'ESP-01 utilise souvent GPIO0 et GPIO2 pour l'I2C en raison de ses broches limitées.
  Wire.begin(0, 2); // Initialise le bus I2C avec SDA sur GPIO0 et SCL sur GPIO2
  ...
  // Inverser l'affichage de 180 degrés (verticalement et horizontalement)
  display.setRotation(2); // 0, 1, 2, 3 pour 0, 90, 180, 270 degrés
  Serial.println(F("Affichage OLED pivoté de 180 degrés."));
